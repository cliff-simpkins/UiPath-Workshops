...
- **`MonsterSelector.uipx`** - solution manifest (generated by `uip agent init`; required by `uip agent push`). Contains local-only project UUIDs, no tenant info.
- **`resources/solution_folder/`** - solution-level resources

`SolutionStorage.json` is intentionally omitted so the starter is tenant-neutral - your first `uip agent push` writes it with your tenant's `SolutionId` and `CloudProjectId`.
